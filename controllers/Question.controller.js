...
exports.deleteQuestion = async (req, res) => {
  try {
    ...
    await Comment.deleteMany({ $or: [{ question: questionId }, { answer: { $in: answerIds } }] });
...
exports.getAllQuestions = async (req, res) => {
  try {
    const questions = await Question.find()
      .populate("user", "fullName username") // Populate user details (fullName & username)
...
exports.getQuestionsByUser = async (req, res) => {
  try {
    ...
    const questions = await Question.find({ user: userId }).populate("answers");
...
exports.voteQuestion = async (req, res) => {
  try {
    ...
    if (question.user.toString() === userId) {
      return res.status(403).json({ success: false, message: "You cannot vote on your own question" });
    }
    ...
    // Downvote logic
      ...
      if (question.downvotes.includes(userId)) {
        return res.status(400).json({ success: false, message: "You have already downvoted this question" });
      }
